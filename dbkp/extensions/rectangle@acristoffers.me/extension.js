...
export default class GnomeRectangle extends Extension {
    ...
    geometryForGrid(geo, index, rowSpan, colSpan, rows, cols) {
    ...
    manage(index, rs, cs, r, c, _t) {
        ...
        if (index >= 0) {
            ...
            if (Math.abs(now - rectangleArgs[5]) < 1000) {
                i = gs.findIndex(g => g[0] == rectangleArgs[0]
                    && g[1] == rectangleArgs[1]
                    && g[2] == rectangleArgs[2]
                    && g[3] == rectangleArgs[3]
                    && g[4] == rectangleArgs[4]);
    ...
    registerShortcut(key, i, rs, cs, r, c) {
